...
train$is_hit <- factor(train$is_hit, levels = c("hit", "nonhit"))
test$is_hit <- factor(test$is_hit, levels = c("hit", "nonhit"))
...
logis_model <- glm(is_hit ~ danceability + energy + loudness + valence +
                     ...
                     acousticness + duration_ms, data = train, family = "binomial")

summary(logis_model)

# Predict on test set
logis_pred <- predict(logis_model, newdata = test, type = "response")
logis_pred_class <- factor(
  ifelse(logis_pred > 0.5, "hit", "nonhit"),
  levels = c("hit", "nonhit")
)

# Confusion matrix
confusionMatrix(
  data = logis_pred_class,
  reference = test$is_hit,
  positive = "hit"
)

# Predicted probability vs. actual hits visualization
log_test <- test |>
  mutate(pred_prob = 1- predict(logis_model, newdata = test, type = "response"),) # adding predictions to the test set

ggplot(log_test, aes(x = pred_prob, fill = is_hit)) +
  geom_density(alpha = 0.6) +
  scale_fill_manual(values = c("nonhit" = "gray", "hit" = "red")) +
...
# Question 3: Can we cluster songs into meaningful groups based on audio features?
